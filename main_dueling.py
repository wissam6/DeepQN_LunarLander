#!/usr/bin/env python3
# DQN + Dueling heads for LunarLander-v3
import argparse
...
class QNetwork(nn.Module):
    """
    Dueling-DQN head:
        shared MLP → Value  V(s)  (1 unit)
                    → Advantage A(s,a) (n_actions units)
        Q(s,a) = V + A − mean(A)
    """
...
class DQNAgent:
    ...
    def select_action(self, state):
        ...
        with torch.no_grad():
            ...
            return int(self.q_net(state).argmax(dim=1).item())
    ...
    def watch(agent, episodes=3):
        # fresh env that opens a Pygame window
        ...
        for ep in range(episodes):
            ...
            while not done:
                ...
                ep_ret += r
                time.sleep(0.03)  # ~30 FPS; adjust as you like
...
def parse_args():
    parser = argparse.ArgumentParser(description="Dueling-DQN for LunarLander-v3")
